...
class darias():
    ...
    def startSimulation(self):
        ...
        SE3_world2target=pin.SE3(np.eye(3),np.array([0.3,0.0,1.4]))
        ...
        for iter in range(0,1000):
            ...
            J = numpy2torch(J)
            J_pesudo = torch.matmul(J.T,torch.inverse(torch.matmul(J,J.T)+1e-3*torch.eye(6)))
...
def main():
    ...
    sim.plotData.plot_ddq(allJoints=True)
